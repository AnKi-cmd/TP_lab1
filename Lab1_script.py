...
input_weight_products = [
    [sg.Text(
        "Введите данные по ВЕСОВЫМ продуктам. В противном случае будут добавлены продукты из приготовленного списка",
...
input_perpiece_products = [
    [sg.Text(
        "Введите данные по ПОШТУЧНЫМ продуктам. В противном случае будут добавлены продукты из приготовленного списка",
        font="35", text_color="green")],
    [sg.Text("Наименование", font="20"), sg.InputText(size=(30, 3), key="product_name", font="35")],
...
class Per_Peace_Products(Products):
    def set_amount(self, amount):
...
class Weignt_Products(Products):
    def set_amount(self, amount):
...
class Buyer(Shop):
    ...
    def get_buy_summ(self):
        summ = 0
        for mes in self.__list_to_buy.keys():
            summ += self.weight_prod.list_of_products[mes]['Цена'] * self.__list_to_buy[mes]
        return round(summ, 2)

    def pay_for_products(self):
        printed = False
        while (self.get_buy_summ() > (self.__balance + self.__bonus_coins)):
            if printed == False:
        ...
        for mes in self.__list_to_buy.keys():
            ...
            cost = self.weight_prod.list_of_products[mes]['Цена']
...
def main():
    ...
    if event == "Confirm":
        window.close()
        person = Buyer(values['FIO'], values['bonus_coins'], values['balance'])
